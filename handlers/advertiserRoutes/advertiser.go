...
func AddAdvertiserRoutes(rg *gin.RouterGroup, db *sqlx.DB) {
	adv := rg.Group("/advertiser")
	repo := repositories.NewAdvertiserRepository(db)

	// получить все Advertiser
	adv.GET("/getAll", func(c *gin.Context) {
		// Создаю контекст
		...
		resp, err := repo.GetAll(timeOutCtx)
		if err != nil {
	...
	adv.PUT("/update", func(c *gin.Context) {
		...
		// Достаю тело запроса
		var jsonReq models.AdvertiserUpdate
		if err := c.ShouldBindJSON(&jsonReq); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
